...
class Efficient(tf.keras.Model):
    ...
    def call(self, inputs, training=None, mask=None, aux_loss=False):
        ...
        #xx = AttentionRefinementModule(x_dil, 144)
...
class Mininet(tf.keras.Model):
    # tarda 50 ms y deberia rtartdar 25
    def __init__(self, num_classes, input_shape=(None, None, 3), weights='imagenet',  norm='selu',
                 conv_func='separable', dropout=0.3, part_conv=True, middle_sum=True,**kwargs ):
        super(Mininet, self).__init__(**kwargs)
        filters = 12

        self.conv1 = Conv_BN(filters=filters, kernel_size=3, strides=2) # 256x128
        self.conv2 = DepthwiseConv_BN(filters*2, kernel_size=3, dilation_rate=1, strides=2)#128x64
        self.conv3 = DepthwiseConv_BN(filters*4, kernel_size=3, dilation_rate=1, strides=2)#64x32
        self.conv4 = DepthwiseConv_BN(filters*8, kernel_size=3, dilation_rate=1, strides=2)#64x32

        self.module1 = Module_MiniNet(filters*8, kernel_size=3, dilation_rate=2, strides=1,
                                      norm=norm, conv=conv_func, dropout=dropout, part_conv=part_conv, middle_sum=middle_sum)#64x32

        self.module2 = Module_MiniNet(filters*8, kernel_size=3, dilation_rate=8, strides=1,
                                      norm=norm, conv=conv_func, dropout=dropout, part_conv=part_conv, middle_sum=middle_sum)#64x32

        self.module3 = Module_MiniNet(filters*8, kernel_size=3, dilation_rate=4, strides=1,
                                      norm=norm, conv=conv_func, dropout=dropout, part_conv=part_conv, middle_sum=middle_sum)#64x32

        self.module4 = Module_MiniNet(filters*8, kernel_size=3, dilation_rate=2, strides=1,
                                      norm=norm, conv=conv_func,  dropout=dropout, part_conv=part_conv, middle_sum=middle_sum)#64x32


        self.down = DepthwiseConv_BN(filters*32, kernel_size=5, dilation_rate=1, strides=3)#64x32

        self.module5 = Module_MiniNet(filters*32, kernel_size=3, dilation_rate=1, strides=1,
                                      norm=norm, conv=conv_func,  dropout=dropout, part_conv=part_conv, middle_sum=middle_sum)#64x32
        self.module6 = Module_MiniNet(filters*32, kernel_size=3, dilation_rate=1, strides=1,
                                      norm=norm, conv=conv_func,  dropout=dropout, part_conv=part_conv, middle_sum=middle_sum)#64x32

        self.conv_down = DepthwiseConv_BN(filters*8, kernel_size=3, dilation_rate=1, strides=2)#64x32

        self.module7 = Module_MiniNet(filters*8, kernel_size=3, dilation_rate=1, strides=1,
                                      norm=norm, conv=conv_func,  dropout=dropout, part_conv=part_conv, middle_sum=middle_sum)#64x32
        self.module8 = Module_MiniNet(filters*8, kernel_size=3, dilation_rate=1, strides=1,
                                      norm=norm, conv=conv_func,  dropout=dropout, part_conv=part_conv, middle_sum=middle_sum)#64x32

        self.trans = Transpose_Conv_BN(filters*2, 5, strides=4)
    ...
    def call(self, inputs, training=None, mask=None, aux_loss=False):
        ...
        x_down = self.conv4(x, training=training)
        x = self.module1(x_down, training=training)
        x = self.module2(x, training=training)
        x = self.module3(x, training=training)
        x_dil = self.module4(x, training=training)

        x_branch2 = self.down(x_down, training=training)
        x_branch2 = self.module5(x_branch2, training=training)
        x_branch2 = self.module6(x_branch2, training=training)
        x_branch2 = self.conv_down(x_branch2, training=training)
        x_branch2 = self.module7(x_branch2, training=training)
        x_branch2 = upsampling(x_branch2, scale=6)


        x = x_dil + reshape_into(x_branch2, x_dil)
        x = upsampling(x, scale=2)
        x = self.module8(x, training=training)

        x = self.trans(x, training=training)
        #x = upsampling(x, scale=4)
...
class Module_MiniNet(tf.keras.Model):
    def __init__(self, filters, kernel_size, strides=1, dilation_rate=1, norm='selu', conv='separable',
                 dropout=0.3, part_conv=True, middle_sum=False):
        super(Module_MiniNet, self).__init__()
        ...
        self.part_conv=part_conv
        self.middle_sum=middle_sum


        if conv == 'separable':
            conv_func = separableConv
        else:
            conv_func = standard_conv

        if part_conv:
            self.conv1 = conv_func(filters=filters, kernel_size=(1, kernel_size), strides=1, dilation_rate=dilation_rate)
            self.conv2 = conv_func(filters=filters, kernel_size=(kernel_size, 1), strides=1, dilation_rate=dilation_rate)
            self.conv3 = conv_func(filters=filters, kernel_size=(kernel_size, 1), strides=1, dilation_rate=dilation_rate)
            self.conv4 = conv_func(filters=filters, kernel_size=(1, kernel_size), strides=strides, dilation_rate=dilation_rate)

        else:
            self.conv1 = conv_func(filters=filters, kernel_size=kernel_size, strides=1, dilation_rate=dilation_rate)
            self.conv2 = conv_func(filters=filters, kernel_size=kernel_size, strides=1, dilation_rate=dilation_rate)
    ...
    def call(self, inputs, training=None):
        if self.part_conv:
            ...
            if self.norm == 'selu':
                x1 = tf.keras.activations.selu(x1)
            ...
            if self.norm == 'selu':
                x2 = tf.keras.activations.selu(x2)
            ...
            if self.norm == 'selu':
                x3 = tf.keras.activations.selu(x3)
            ...
            if self.dropout:
                x2 = self.dropout_layer(x2, training=training)

            x2 += inputs

            if self.norm == 'bn':
                x = layers.LeakyReLU(alpha=0.3)(x2)
            if self.norm == 'selu':
                x = tf.keras.activations.selu(x2)
